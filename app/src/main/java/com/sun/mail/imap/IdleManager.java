...
public class IdleManager {
                ...
                try {
                    try {
                        try {
                            ...
                            if (this.logger.isLoggable(Level.FINEST)) {
                                this.logger.log(Level.FINEST, "IdleManager selected {0} channels", Integer.valueOf(iSelect));
                            }
    ...
    private void watchAll() {
        while (true) {
            ...
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager adding {0} to selector", folderName(iMAPFolderPoll));
            }
            try {
    ...
    private void processKeys() throws IOException {
        ...
        while (true) {
            ...
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager aborting IDLE for folder: {0}", folderName(iMAPFolderPoll));
            }
            SocketChannel channel = iMAPFolderPoll.getChannel();
            if (channel != null) {
    ...
    private void unwatchAll() {
        ...
        while (true) {
            ...
            if (this.logger.isLoggable(Level.FINEST)) {
                this.logger.log(Level.FINEST, "IdleManager aborting IDLE for unwatched folder: {0}", folderName(iMAPFolderPoll));
            }
            SocketChannel channel = iMAPFolderPoll.getChannel();
            if (channel != null) {
                try {
