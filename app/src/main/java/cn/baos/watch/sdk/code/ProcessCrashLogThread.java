...
import cn.baos.watch.sdk.code.mail.SendMailCallback;
import cn.baos.watch.sdk.code.mail.SendMailUtil;
...
public class ProcessCrashLogThread extends Thread {
    ...
    public void run() throws Throwable {
        ...
        if (!file.exists() || (fileArrListFiles = file.listFiles(new FileFilter() { // from class: cn.baos.watch.sdk.code.ProcessCrashLogThread.1
        ...
        for (final File file2 : fileArrListFiles) {
            try {
                SendMailUtil.send("APP奔溃了: " + file2.getName().replace("crash-", "").replace(".log", ""), readFile(file2), new SendMailCallback() { // from class: cn.baos.watch.sdk.code.ProcessCrashLogThread.2
                    @Override // cn.baos.watch.sdk.code.mail.SendMailCallback
